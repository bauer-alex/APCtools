...
```{r ridgeline matrix}
dist_breaks <- c(1,500,1000,2000,6000,25000)

plot_density(data_RA, y_var = "JS_HUR_Reisedistanz", log_scale = TRUE, weights_var = "G_Gewichtung")
plot_density(data_RA, y_var = "JS_HUR_Reisedistanz", y_var_breaks = dist_breaks,
             weights_var = "G_Gewichtung", log_scale = TRUE)
plot_density(data_RA, y_var = "JS_HUR_Reisedistanz", log_scale = TRUE, plot_type = "boxplot", weights_var = "G_Gewichtung")
...
plot_densityMatrix(data_RA, y_var = "JS_HUR_Reisedistanz",
                   age_groups = age_groups, period_groups = period_groups,
                   log_scale = TRUE, y_var_breaks = dist_breaks)

plot_densityMatrix(data_RA, y_var = "JS_HUR_Reisedistanz", dimensions = c("period","cohort"),
                   period_groups = period_groups, cohort_groups = cohort_groups,
                   log_scale = TRUE, y_var_breaks = dist_breaks)

plot_densityMatrix(data_RA, y_var = "JS_HUR_Reisedistanz", plot_type = "boxplot",
                   age_groups = age_groups, period_groups = period_groups,
                   log_scale = TRUE)

plot_densityMatrix(data_RA, y_var = "S_Haushaltsgroesse",
                   age_groups = age_groups, period_groups = period_groups)
```
