...
plot_variable(dat = data_RA, variable = "mainTrip_distance", apc_dimension = "period", log_scale = TRUE)

# plot a categorical variable
plot_variable(dat = data_RA, variable = "household_size", apc_dimension = "period")
plot_variable(dat = data_RA, variable = "household_size", apc_dimension = "period", geomBar_position = "stack")
```
...
```{r}
model_pure <- gam(mainTrip_distance ~ te(age, period), data = data_RA)
...
```{r}
APCtools::plot_partialAPCeffects(model_pure, dat = data_RA, variable = "period")
APCtools::plot_marginalAPCeffects(model_pure, dat = data_RA, variable = "cohort",
...
```{r}
# estimated heatmap
APCtools::plot_APCheatmap(model_pure, dat = data_RA, plot_CI = FALSE, bin_heatmap = FALSE)

# binned heatmap
APCtools::plot_APCheatmap(model_pure, dat = data_RA, plot_CI = FALSE)
...
APCtools::plot_APCheatmap(model_pure, dat = data_RA, dimensions = c("cohort","age"), plot_CI = FALSE)
...
APCtools::plot_APCheatmap(model_pure, dat = data_RA, plot_CI = T, bin_heatmap = FALSE,
                          markLines_list = list(age = c(25,35,70)), dimensions = c("period","cohort"))

# manual binning
manual_binning <- list(period = seq(min(data_RA$period, na.rm = TRUE) - 1, max(data_RA$period, na.rm = TRUE), by = 5),
                       cohort = seq(min(data_RA$period - data_RA$age, na.rm = TRUE) - 1, max(data_RA$period - data_RA$age, na.rm = TRUE), by = 10))
APCtools::plot_APCheatmap(model_pure, dat = data_RA, plot_CI = FALSE,
